...
def test_accept_invitation(client: TestClient):
    ...
    supervisor_user = client.post(
        "/user/", headers={"cred": "supervisor"}, json=body
    ).json()
    supervised_user = client.post(
        "/user/", headers={"cred": "supervised"}, json=body
    ).json()

    # Aceptamos las invitaciones
    # Acá, test acepta la invitación de supervisor
    test1_response = client.post(
        f"/supervisors/invitation?code={supervisor_user['invitation']}",
        headers={"cred": "test"},
    )
    # Acá, el supervisado acepta la invitación de test
    test2_response = client.post(
        f"/supervisors/invitation?code={test_user['invitation']}",
...
def test_delete_supervisor(client: TestClient):
    ...
    response = client.delete(
        f"/supervisors/supervisor/{supervisor_user['_id']}", headers={"cred": "test"}
    )
...
def test_delete_supervised(client: TestClient):
    ...
    response = client.delete(
        f"/supervisors/supervised/{supervised_user['_id']}", headers={"cred": "test"}
    )
